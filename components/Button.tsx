...
function Button({ title, onClick, width, loading, padding, noIcon }: Props) {
  return (
    <button
      className={`ease z-60 group relative box-border inline-flex ${
        width ? width : 'w-auto'
      } ${padding} cursor-pointer items-center justify-center overflow-hidden rounded bg-violet-600 bg-gradient-to-r from-red-500 to-blue-500 px-8 py-3 font-bold text-white transition-all duration-300 focus:outline-none`}
      onClick={onClick}
    >
      ...
      <span className="z-60 relative flex items-center font-semibold">
        {noIcon && (
          <svg
